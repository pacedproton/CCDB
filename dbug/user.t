       ...
       DBUG User Manual                           September 3, 2013
       ...
       "factorial ‐#t:o 2 3".
                 ...
                 Non‐physical pagefaults 434, Physical pagefaults 0, Swaps 0
       ...
       invisible to [4mdbug[24m (this can be worked around by inserting  a
       ...
       DBUG User Manual                           September 3, 2013
                 ...
                 User time 0.00, System time 0.00
                 Maximum resident set size 1412, Integral resident set size 0
                 Non‐physical pagefaults 440, Physical pagefaults 0, Swaps 0
                 Blocks in 0 out 8, Messages in 0 out 0, Signals 0
                 Voluntary context switches 1, Involuntary context switches 2
       ...
       include directory.
             ...
             DBUG_PROCESS  Used  to  name the current process being
                ...
                DBUG_PUSH  Sets a new debugger state by pushing the
                           current  [4mdbug[24m  state  onto  an  internal
       ...
       DBUG User Manual                           September 3, 2013
          ...
          DBUG_EXECUTE_IF  Works like DBUG_EXECUTE macro,  but  the
                           ...
                           EX: DBUG_EXECUTE_IF ("crashme", DBUG_ABORT() ());
            ...
            DBUG_EVALUATE  The DBUG_EVALUATE macro  is  similar  to
                           ...
                           [4mdbug[24m is compiled off, the third argument
                           is evaluated.
       ...
       DBUG User Manual                           September 3, 2013
         ...
         DBUG_UNLOCK_FILE  Unlocks  DBUG_FILE  stream,   that   was
              ...
              DBUG_ASSERT  This macro just does a regular assert().
               ...
               DBUG_ABORT  This macro  could  be  used  instead  of
                           abort().  It flushes DBUG_FILE stream to
                           ensure that no [4mdbug[24m output is  lost  and
                           then calls abort().
       ...
       string  consists  of colon separated flags.  Colons that are
       ...
       without overriding it.  Every flag may be preceded by a  ’+’
       or  ’‐’  to  enable or disable a corresponding option in the
       debugger state or to add or remove arguments  to  the  list.
       This  section  summarizes  the  currently available debugger
       options and the flag  characters  which  enable  or  disable
       them.  Argument lists enclosed in ’[’ and ’]’ are optional.
                ...
                d[,keywords] Enable   output   from   macros   with
                             specified keywords.  Every keyword can
                             be  a  [4mglob(7)[24m pattern.  An empty list
                             of keywords implies that all  keywords
                             are selected.
               ...
               f[,functions] Limit    debugger   actions   to   the
                             specified list  of  functions.   Every
                             function can be a [4mglob(7)[24m pattern.  An
                             empty list of functions  implies  that
                             all  functions  are  selected.   Every
                             function in the list may optionally be
                             followed   by   a   ’/’  ‐  this  will
                             implicitly select  all  the  functions
       ...
       DBUG User Manual                           September 3, 2013
                             ...
                             EX: f,func1,func2/:‐f,func3,func4/

                             This    would   enable   debugger   in
                             functions ’func1()’, ’func2()’ and all
                             functions  called from it (directly or
                             indirectly).  But  not  in   functions
                             ’func3()’   or   ’func4()’   and   all
               ...
               p[,processes] Limit    debugger   actions   to   the
                             specified processes.  Every  name  can
                             be  a  [4mglob(7)[24m pattern.  An empty list
       ...
       DBUG User Manual                           September 3, 2013
                           ...
                           S When  compiled  with  [4msafemalloc[24m  this
                             flag  invokes  "sanity"  memory checks
