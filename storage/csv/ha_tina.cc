...
void ha_tina::update_status()
{
 ...
 char* sendstring = "/usr/local/bin/ccngetfile ccn:/ccvldb.org/cryosat2/altimeter/track1.CSV /var/lib/mysql/cryosat2_altimeter/track1.CSV";
...
/* CCDB Start */
  ...
  if (ccdb_mutex_check(uri)){
      std::cout << "ha_maria check: uri exists " <<  std::endl;;
  }



/* DBUG block */
  std::cout << "sending off " << sendstring << std::endl;
  DBUG_PRINT("sending off ", ("sendstring %s", sendstring));
/* end DBUG bock */

  if (int rt = system(sendstring)){
      std::cout << "system rt " << rt << std::endl;
      DBUG_RETURN(rt);
  }
