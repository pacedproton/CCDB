...
end_with_restore_list:
  ...
  */
  ...
  case SQLCOM_SHOW_AUTHORS:
    ...
    /* CCDB commands start cases  */

  case SQLCOM_CCDB_PUBLISH_STATUS:
      res = ccdb_publish_status_command(thd, parameter);
      break;

  case SQLCOM_CCDB_OPENDB:
      res = ccdb_opendb_command(thd);
      break;

  case SQLCOM_CCDB_CLOSEDB:
      res = ccdb_closedb_command(thd);
      break;

  case SQLCOM_CCDB_PORT:
      res = ccdb_port_command(thd);
      break;

   case SQLCOM_CCDB_OPENTABLE:
      res = ccdb_opentable_command(thd, parameter);

      break;

  case SQLCOM_CCDB_SHOW_CCTABLES:
      res = ccdb_show_cctables_command(thd);
      break;

  case SQLCOM_CCDB_SHOW_STATUS:
      res = ccdb_show_status_command(thd);
      break;

  case SQLCOM_CCDB_PUBLISH:
      res = ccdb_publish_command(thd, parameter);
      break;

  case SQLCOM_CCDB_LOCK_CCTABLE:
      res = ccdb_lock_cctable_command(thd, parameter);
      break;

  case SQLCOM_CCDB_UNLOCK_CCTABLE:
      res = ccdb_unlock_cctable_command(thd, parameter);
      break;
