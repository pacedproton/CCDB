...
/* Start CCDB includes */ 
#include "/opt/mariadb.git/trunk/storage/csv/ccdb.h"
...
const char* zookeeper_hostnameport = "127.0.0.1:2181";
const char* zookeeper_mutex_node = "/ccdb/mutexes";
const char* fs = "---";
...
bool ccdb_publish_status_command(THD *thd, const char *ttfp)
{

    LEX *lex = thd->lex;
//    std::string ttfpstring = lex->select_olex.db;
    std::string ttfpstring = lex->name.str;

    std::cout << "show ccdb_publish_status_command paramter received " << ttfpstring << std::endl;
    
    // workaround
//    std::string ttfpstring = 

    CCDB_Global_Status ccdb_Global_Status_Single;
    ccdb_Global_Status_Single.showSinglePublishStatus(ttfpstring);

    int id=0;


    std::stringstream out;
    out << ccdb_Global_Status_Single.childrenCount;
    std::string clientString = out.str();

    cout << clientString;

    List<Item> field_list;
    Protocol *protocol = thd->protocol;
    DBUG_ENTER("ccdb_show_cctables_command");

    field_list.push_back(new Item_empty_string("CCDB Table", 50));
    field_list.push_back(new Item_empty_string("Lock Issued To", 12));
    field_list.push_back(new Item_empty_string("Repository Node", 12));
    field_list.push_back(new Item_empty_string("Published", 1));
//    field_list.push_back(new Item_return_int("No of Tables", 7, MYSQL_TYPE_LONGLONG));
    ...
    if (protocol->send_result_set_metadata(&field_list,
     ...
     protocol->store(ccdb_Global_Status_Single.ccdbMutexes[0].uri.c_str(), system_charset_info);
     protocol->store(ccdb_Global_Status_Single.ccdbMutexes[0].user.c_str(), system_charset_info);
     protocol->store(ccdb_Global_Status_Single.ccdbMutexes[0].node.c_str(), system_charset_info);
     protocol->store(ccdb_Global_Status_Single.ccdbMutexes[0].published.c_str(), system_charset_info);
//	protocol->store((longlong)debugoutput);
...
bool ccdb_lock_cctable_command(THD *thd, const char *ttfp){


    LEX *lex = thd->lex;
    std::string ttfpstring = lex->name.str;
   
    std::string opentableUri = ReplaceString(ttfpstring, "$", ":");

    if (ccdb_mutex_check(opentableUri)){
	std::cout << "sql_show: uri exists " <<  std::endl;
    }

    List<Item> field_list;
    Protocol *protocol = thd->protocol;
    DBUG_ENTER("ccdb_ccdb_lock_command");

    field_list.push_back(new Item_empty_string("CCDB Table", 60));
    field_list.push_back(new Item_empty_string("Operation", 12));
    field_list.push_back(new Item_empty_string("Read-back Lock State", 12));
    ...
    protocol->prepare_for_resend();
    protocol->store(opentableUri.c_str(), system_charset_info);
    protocol->store("Table Lock", system_charset_info);
    protocol->store("Locked", system_charset_info);
    if (protocol->write())
	DBUG_RETURN(TRUE);

    DBUG_PRINT("ccdb_lock_cctable with", ("uri %s", opentableUri.c_str()));
...
bool ccdb_unlock_cctable_command(THD *thd, const char *ttfp){
    ...
    List<Item> field_list;
    Protocol *protocol = thd->protocol;
    DBUG_ENTER("ccdb_ccdb_unlock_command");
    ...
    protocol->prepare_for_resend();
    protocol->store(opentableUri.c_str(), system_charset_info);
    protocol->store("Table Unlock", system_charset_info);
    protocol->store("Unlocked", system_charset_info);
    if (protocol->write())
	DBUG_RETURN(TRUE);

    DBUG_PRINT("ccdb_unlock_cctable with", ("uri %s", opentableUri.c_str()));
...
bool ccdb_show_cctables_command(THD *thd){
    ...
    protocol->prepare_for_resend();
    protocol->store("testtable", system_charset_info);
    protocol->store("testuser", system_charset_info);
    protocol->store("testnode", system_charset_info);
    if (protocol->write())
	DBUG_RETURN(TRUE);
	...
	string uri = "ccnx://ccdb/mutexes/ccdb_Table1";
	string node ="vega";
	string user ="ccdbuser";

    std::cout << "calling mutex_check() with uri 2:   " << uri << std::endl;

    char* retc = ccdb_mutex_check(uri);

/* DBUG block */
    std::cout << "mutex_check return code" << retc << std::endl;
    DBUG_PRINT("show cctables with", ("uri %s", uri.c_str()));
    std::cout << "show_cctables exit" << std::endl;
/* end DBUG bock */ 
...
bool ccdb_show_status_command(THD *thd){


    CCDB_Global_Status ccdb_Global_Status;
    ccdb_Global_Status.showStatus("ccnx:/ccvldb.org/cryosat2/altimeter/track1");
    int debugoutput = ccdb_Global_Status.childrenCount;
    int id=0;
    cout << '\n'<< "test from CCDB_Publish_Status.children_count "  
	 << debugoutput <<std::endl;  
    
    cout << "from object vector struct: " << ccdb_Global_Status.ccdbMutexes[1].published 
		 << "node: " << ccdb_Global_Status.ccdbMutexes[1].node
		 << "user: " << ccdb_Global_Status.ccdbMutexes[1].user << std::endl;


    std::stringstream out;
    out << ccdb_Global_Status.childrenCount;
    std::string clientString = out.str();
...
//    field_list.push_back(new Item_return_int("No of Tables", 7, MYSQL_TYPE_LONGLONG));
    ...
    while (id < ccdb_Global_Status.childrenCount){

        protocol->prepare_for_resend();
	protocol->store(ccdb_Global_Status.ccdbMutexes[id].uri.c_str(), system_charset_info);
	protocol->store(ccdb_Global_Status.ccdbMutexes[id].user.c_str(), system_charset_info);
	protocol->store(ccdb_Global_Status.ccdbMutexes[id].node.c_str(), system_charset_info);
	protocol->store(ccdb_Global_Status.ccdbMutexes[id].published.c_str(), system_charset_info);
//	protocol->store((longlong)debugoutput);
    ...
    string uriccdbstatus = "ccn:/ccvldb.org/cryosat2/altimeter/track1";

    std::cout << "calling ccdb_show_status() with uri: " << uriccdbstatus << std::endl;
    int children_count=0;

    char* retc = ccdb_show_status(uriccdbstatus);

    std::cout << "number of node children sql_show: " << children_count << std::endl;

/* DBUG block */
    std::cout << "ccdb_show_status return code" << retc << std::endl;
    DBUG_PRINT("show status with", ("uri %s", uriccdbstatus.c_str()));
    std::cout << "ccdb_show_status exit" << std::endl;
/* end DBUG bock */ 
...
bool ccdb_opendb_command(THD *thd){ return true;}
bool ccdb_closedb_command(THD *thd){ return true;}
bool ccdb_port_command(THD *thd){ return true;}
bool ccdb_opentable_command(THD *thd, const char *ttfp){ 

    LEX *lex = thd->lex;
    std::string ttfpstring = lex->select_lex.db;

    std::cout << "ccdb_opentable paramter *ttfp received uri from caller: " << ttfpstring << std::endl;
   
    std::string opentableUri = ReplaceString(ttfpstring, "$", ":");
    std::cout << "ccdb_opentable paramter *ttfp received uri from caller: " << opentableUri << std::endl;

    system("/usr/local/bin/ccngetfile ccn:/ccvldb.org/cryosat2/altimeter/lrm.CSV /var/lib/mysql/cryosat2_altimeter/lrm.CSV");
    ...
    protocol->prepare_for_resend();
    protocol->store(opentableUri.c_str(), system_charset_info);
    protocol->store("CCNx Open OK", system_charset_info);
    protocol->store("Locked", system_charset_info);
    if (protocol->write())
	DBUG_RETURN(TRUE);


	string uritest = "ccnx://ccdb/mutexes/ccdb_Table1";
	string node ="vega";
	string user ="ccdbuser";

    std::cout << "calling mutex_check() with uri 2:   " << uritest << std::endl;

    char* retc = ccdb_mutex_check(uritest);

/* DBUG block */
    std::cout << "mutex_check return code" << retc << std::endl;
    DBUG_PRINT("show cctables with", ("uri %s", uritest.c_str()));
    std::cout << "show_cctables exit" << std::endl;
/* end DBUG bock */ 
    ...
    std::cout << "greetings from ccdb_opentable_command " << std::endl;
...
bool ccdb_publish_command(THD *thd, const char *ttfp){ 

    LEX *lex = thd->lex;
    std::string ttfpstring = lex->name.str;

    std::cout << "ccdb_publish paramter *ttfp received uri from caller: " << ttfpstring << std::endl;
   
    std::string opentableUri = ReplaceString(ttfpstring, "$", ":");
    std::cout << "ccdb_publish paramter *ttfp received uri from caller: " << opentableUri << std::endl;

    system("ccnputfile -allownonlocal ccnx:/ccvldb.org/cryosat2/altimeter/lrm2.CSV /var/lib/mysql/cryosat2_altimeter/lrm.CSV");
    ...
    protocol->prepare_for_resend();
    protocol->store(opentableUri.c_str(), system_charset_info);
    protocol->store("CCNx Repo Publish OK", system_charset_info);
    protocol->store("Unlocked", system_charset_info);
    if (protocol->write())
	DBUG_RETURN(TRUE);
